...
async function run() {
  ...
  const serverBinary = path.join(
    ...
    "target",
    "debug",
    process.platform === "win32" ? "cratestack-lsp.exe" : "cratestack-lsp",
  );
  await vscode.workspace
    .getConfiguration("cratestack")
    .update("lsp.path", serverBinary, vscode.ConfigurationTarget.Global);

  // Derived from package.json rather than hardcoded: this was pinned to the
  // pre-rename `vaam-store.` publisher, so the lookup returned undefined and
  // the assertion below could only ever fail. Nothing caught it because this
  // suite is not wired into CI (`test:vscode-smoke` needs a real VS Code
  // download and a built cratestack-lsp).
