...
- **TypeScript**: `Decimal`-typed fields are now `decimal.js`'s `Decimal` class (re-exported from the generated `models.ts` as a `DecimalJs.clone({ toExpNeg: -1e9, toExpPos: 1e9 })` — an unbounded-exponent clone, so `.toString()`/`.toJSON()` always emit plain positional notation too, for the same reason as Dart's `.toString()` choice). `decimal.js` is a `dependencies` entry (not `peerDependencies`, unlike `@tanstack/react-query`) in every generated `package.json` — every consumer needs a working `Decimal` implementation and there's no app-owned-singleton constraint the way there is for React/react-query, so nothing is gained by pushing the choice onto the app. Adds ~32 KB minified / ~13 KB minified+gzip, zero transitive dependencies (measured: `npm view decimal.js dist.unpackedSize` reports 284 KB unpacked; a local `terser` minify of the runtime file alone is 32,328 bytes, 12,860 gzipped).
  ...
  Unlike Dart, this package had **no decode/encode chokepoint at all** before this change — every response was a blind `JSON.parse`/codec-decode cast with `as T`, no runtime transform of any kind (a `DateTime` field is still a bare `string`, by design). A real `Decimal` instance needs an actual runtime replace of the wire string, so `models.ts.j2` gains a `reviveDecimalFields(value, decimalKeys)` helper (walks a decoded response tree, replacing a string at any key named in `decimalKeys` with `new Decimal(...)`), and the generated `default`-preset REST (`rest-client.ts.j2`) and RPC (`rpc-client.ts.j2`) clients' `list`/`get`/`create`/`update` methods call it unconditionally (an empty key set — every model without a `Decimal` field — is a documented fast-path no-op, so this is a uniform, always-present `.then(...)` wrapper rather than the generator branching per model). `decimalKeys` is computed **per model, from that model's own direct fields only** — deliberately not recursive into a relation-embedded field's own model (a `Post.author.balance`-shaped case), which sidesteps the field-name-collision risk a schema-global key set would have while covering the ticket's central case (a flat `Decimal` field on a model) completely. `Decimal.prototype.toJSON` (an alias for `.toString()`) makes the *encode* direction (`JSON.stringify`, which both a REST request body and this package's default `jsonRpcCodec` go through) work automatically, with no generated glue needed. `DecimalFilter`'s comparison operands are `ComparableFilter<Decimal>` now; they never need decode-side revival since a `Where`/`FindMany` argument only ever travels outbound.

  **Two scope narrowings, not silent gaps:** (1) the TypeScript `swr` preset's generated per-model decode functions (`models-rest.ts.j2`/`models-rpc.ts.j2`) don't call `reviveDecimalFields` yet — the type flows through correctly (compiles, `DecimalFilter` encodes correctly) but decode-side revival is a follow-up; (2) a user-defined `procedure`'s return type isn't revived even when it structurally contains a `Decimal` field — only the generated model CRUD methods are, in both transports.

- **gRPC (both SDKs), deliberately out of scope for #498 but verified not broken:** `grpc/wire.rs` is unchanged — `Decimal` still maps to proto3 `string` (`GrpcWireKind::String`) on the wire, same as before. Dart's gRPC preset reuses the identical `Model.fromWire`/`.toWire()` factories the REST/RPC presets use (confirmed by generating a `transport grpc` schema with a `Decimal` field and running `flutter analyze` — clean), so it's not just non-broken, it's *correct*: proto3 carries a string, the in-memory value is a real `Decimal`. TypeScript's gRPC-Web preset has no equivalent decode/encode chokepoint (see above) — a probe schema (`transport grpc`, a `Decimal` field, real `.pb.lock`) generates and `tsc`-typechecks cleanly, but the decoded value at a `Decimal`-typed field is actually still a raw JS `string` at runtime (`grpc-web-runtime.ts.j2`'s `decodeMessage` never gained a `Decimal`-aware case), a real static-type/runtime mismatch — **filed as a follow-up rather than silently left**, since fixing it is a genuine new feature (gRPC-Web `Decimal` decoding), not a regression this change introduced.
