...
--preset swr` client with **zero hand-written data-fetching code** — every `useSWR`/`useSWRMutation`
call, cache key, and fetcher in `web/src` comes from the generator, not from this app.
...
- **A real Postgres-backed REST server** (`src/lib.rs`, `schema.cstack`): two models with a
  ...
  single-model fixture, without a `@@paged` model (the `swr` preset has a known gap there — see
  `docs`) and without a `to_camel_case` model-name collision (the default preset's own known gap).
...
- **A React app** (`web/src`) that only imports generated code:
  - `useBoards`/`useCreateBoard` (`react-vite-swr-client/models/board.hooks`) — list read + a
    mutation whose invalidation visibly refreshes that same list.
  - `useBoard`/`useTasks`/`useCreateTask`/`useUpdateTask`/`useDeleteTask`
    (`react-vite-swr-client/models/task.hooks`) — a detail read, a list read, and the full CRUD
    mutation set, each invalidating per the generated hooks' fixed rule.
  - `useEstimateFocusMinutesQuery` (`react-vite-swr-client/procedures.hooks`) — a query-kind
...
| [`client/`](client) | Generated `--preset swr` package (checked in) |
...
## Scope / known gaps carried in from the preset

- The `swr` preset doesn't support `@@paged` models yet — this schema doesn't use one, on purpose.
...
- The sibling-hooks-file subpath-import pattern (`.../models/board.hooks`) works as designed once
  `package.json` actually declares the subpath in `exports` — it didn't (fixed as part of this PR).
