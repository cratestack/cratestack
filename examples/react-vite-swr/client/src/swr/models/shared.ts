...
// `reviveWireFields` on every decoded response too.
...
/** See the default preset's `models.ts.j2` for the full doc comment on
 *  why `null`/non-array values pass through untouched — identical
 *  implementation. */
 ...
 *  functions can revive a `Decimal`/`Bytes`-typed (rather than
 *  object-typed) return value. */
