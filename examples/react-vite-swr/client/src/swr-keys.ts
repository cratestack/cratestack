...
export const swrKeys = {
  model: {
    Board: {
      list: (query?: CratestackFetchQuery) => ["/boards", "list", query] as const,
      get: (id: number | null | undefined, query?: CratestackFetchQuery) =>
        id == null ? null : (["/boards", "detail", id, query] as const),
      listMatches: (key: unknown): boolean =>
        Array.isArray(key) && key[0] === "/boards" && key[1] === "list",
      create: () => ["/boards", "create"] as const,
      update: (id: number) => ["/boards", "update", id] as const,
      delete: (id: number) => ["/boards", "delete", id] as const,
    },
    Task: {
      list: (query?: CratestackFetchQuery) => ["/tasks", "list", query] as const,
      get: (id: number | null | undefined, query?: CratestackFetchQuery) =>
        id == null ? null : (["/tasks", "detail", id, query] as const),
      listMatches: (key: unknown): boolean =>
        Array.isArray(key) && key[0] === "/tasks" && key[1] === "list",
  ...
  procedure: {
    estimateFocusMinutes: (args: EstimateFocusMinutesArgs | null | undefined) =>
      args == null ? null : (["/$procs/estimateFocusMinutes", args] as const),
  },
} as const;