...
/// `flutterRiverpodClientAdapterProvider` with — the *existing*,
...
/// `BoardApi.list()` method itself ran (not
...
void main() {
  ...
  test(
    'overriding flutterRiverpodClientAdapterProvider alone reaches boardListProvider '
    ...
    () async {
      ...
      // `boardListProvider` now always takes an
      // optional `query` (issue #331), so `riverpod_generator` emits it
      // as a family — even the zero-argument default has to be called,
      // `boardListProvider()`, not read bare.
      final result = await container.read(boardListProvider().future);
  ...
  test(
    'a non-default CratestackListQuery passed to boardListProvider reaches the '
    ...
    () async {
      ...
      final result = await container.read(boardListProvider(query: query).future);
  ...
  test(
    'boardListProvider caches by query value, not identity '
    ...
    () async {
      ...
      await container.read(boardListProvider(query: queryA).future);
      await container.read(boardListProvider(query: queryB).future);
