...
Model CRUD only — procedures and server-streaming aren't wired into the generated tonic service yet (`cratestack-grpc`'s own scope note), so none of the three clients has a method for them either. `list()` on all three clients supports `limit`/`offset`/`fields`/`include`/`sort`; raw predicate queries (`where`/`or`) and structured filters aren't wired up in any generator yet.
