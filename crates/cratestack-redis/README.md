...
`EXPIRE` bounds how long a bucket lives, not how many a caller can create — and `RateLimitLayer` runs before authentication, so a caller rotating an unverified `Authorization` header used to mint one `:rl:` key per request. `consume_bounded` closes that in the **same** Lua script, with no extra round-trip: it additionally takes a scope set at `<prefix>:rls:<sha256(scope)>` and a fallback bucket key. On first sight of a bucket under a scope it `SADD`s it when `SCARD < max_distinct`, and otherwise charges the *fallback* bucket. Set members are the first 16 hex chars of the bucket hash.

The scope set is `PEXPIRE`d to `cratestack_core::scope_ttl_secs` — never shorter than the bucket TTL — on **every** admission, so the admission record always outlives the buckets it admitted and no second generation can open underneath the first. (An earlier revision suffixed the key with a fixed-window epoch and expired it after the window; with `window < bucket_ttl` that produced `max_distinct × ceil(bucket_ttl / window)` buckets per scope, not `max_distinct`.) Re-`PEXPIRE`ing on every admission also repairs a set left without a TTL by a script that aborted between `SADD` and `PEXPIRE`: Lua here is atomic but not transactional, so a mid-script `OOM` can stop it partway. Steady-state keyspace is O(scopes × max_distinct) rather than O(requests).
