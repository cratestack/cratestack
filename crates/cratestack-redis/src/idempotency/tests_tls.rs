...
fn open_with_tls_accepts_rediss_scheme_with_system_trust_store() {
    let store = RedisIdempotencyStore::open_with_tls("rediss://127.0.0.1:6380", "bank", no_certs());
    assert!(
        store.is_ok(),
...
fn open_with_tls_accepts_a_private_ca_bundle() {
    ...
    let store = RedisIdempotencyStore::open_with_tls("rediss://127.0.0.1:6380", "bank", certs);
    assert!(
        store.is_ok(),
...
fn open_with_tls_rejects_non_tls_scheme() {
    let store = RedisIdempotencyStore::open_with_tls("redis://127.0.0.1:6379", "bank", no_certs());
    assert!(
        store.is_err(),
