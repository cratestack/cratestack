...
async fn connection_is_lazy_and_cached_after_first_success() {
    let store = RedisIdempotencyStore::open("redis://127.0.0.1:1", "cratestack:test:idem-lazy")
        .expect("open");
    assert!(
...
async fn failed_connection_attempt_is_not_cached_so_next_call_can_retry() {
    ...
    let store = RedisIdempotencyStore::open("redis://127.0.0.1:1", "cratestack:test:idem-retry")
        .expect("open");
...
async fn connection_is_reused_not_reopened_per_call() {
    ...
    let store =
        RedisIdempotencyStore::open(redis.url.clone(), "cratestack:test:idem-reuse-identity")
            .expect("open");
