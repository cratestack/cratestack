...
/// the decode — which is why a substring test on the path is good enough
/// here and no `MatchedPath` plumbing is warranted.
...
/// `cratestack_core::rpc`'s `RPC_*_PATH`). Matched as a substring rather
/// than a prefix on purpose: the RPC router is routinely nested under an
/// application prefix (`/api/rpc/...`), and a prefix test would silently
/// hand those deployments the REST vocabulary.
