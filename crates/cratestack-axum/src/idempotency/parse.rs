...
pub fn parse_idempotency_key(headers: &http::HeaderMap) -> Result<Option<String>, CratestackError> {
    let Some(value) = headers.get("idempotency-key") else {
        return Ok(None);
    };
    let raw = value
        .to_str()
        .map_err(|_| CratestackError::BadRequest("Idempotency-Key must be ASCII".to_owned()))?
        .trim();
    if raw.is_empty() {
