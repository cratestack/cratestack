...
- `lib/src/apis.dart`
- `example/main.dart`
- `test/tiny_rest_client_test.dart`

Import only `package:tiny_rest_client/tiny_rest_client.dart` from application code. Files under `lib/src/` are implementation details.
...
```dart
final client = TinyRestClientCratestackClient(
  myAdapter,
  basePath: '/api',
);
```
...
- `tinyRestClientAdapterProvider`
- `tinyRestClientBasePathProvider`
- `tinyRestClientClientProvider`
- `tinyRestClientWidgetApiProvider`
- `tinyRestClientProceduresApiProvider`
...
final container = ProviderContainer(
  overrides: [
    tinyRestClientAdapterProvider.overrideWithValue(
      CratestackDioAdapter(dio: myDio, useRustTransport: true),
    ),
    tinyRestClientBasePathProvider.overrideWith((ref) => '/api'),
  ],
);
```
...
```dart
final tinyRestClientWidgetListProvider = FutureProvider((ref) async {
  final client = ref.watch(tinyRestClientClientProvider);
...
```dart
final tinyRestClientWidgetCardProvider = FutureProvider.family((ref, int id) async {
  final client = ref.watch(tinyRestClientClientProvider);
  final selection = WidgetSelection()
    ..id()
;
...
```dart
final tinyRestClientWidgetPagedProvider = FutureProvider((ref) async {
  final client = ref.watch(tinyRestClientClientProvider);
  final selection = WidgetSelection()
    ..id()
;
...
Widget buildPagedList(WidgetRef ref) {
  final page = ref.watch(tinyRestClientWidgetPagedProvider);
...
```dart
final tinyRestClientWidgetProjectedPageProvider = FutureProvider((ref) async {
  final client = ref.watch(tinyRestClientClientProvider);
  final selection = WidgetSelection()
    ..id()
;
...
class WidgetCardView extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final item = ref.watch(tinyRestClientWidgetCardProvider(id));
