...
//! CRUD-only gRPC client touches: each model's response shape (recursing
//! into relations/`type` blocks via the collector), `Create<M>Input`/
//! `Update<M>Input`, the per-model RPC wrapper inputs
...
pub(crate) fn build_grpc_messages(
...
) -> Result<Vec<GrpcMessageView>, DartGeneratorError> {
    ...
    // too, and `models.dart.j2`'s hand-written `PageInfo` class declares
    // them `required`, not nullable) — see
    // `GrpcFieldDescriptor::defaults_when_absent`'s doc.
    ...
    for model in models_with_pk {
        ...
        if crate::naming::model_allows_create(model) {
            collect_from_fields(
        ...
        let pk = crate::naming::primary_key_field(model)
