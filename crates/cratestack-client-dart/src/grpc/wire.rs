//! Field-level wire-kind mapping — the Dart-generation-time mirror of
//! `cratestack-proto::emit::scalar::map_scalar` (`docs/design/protobuf.md`
//! §4.1). Reimplemented locally (not depended on) for the same reason
//! `cratestack-client-typescript::grpc::wire` reimplements it rather than
//! reaching into `cratestack-proto::emit`'s private module: the
//! "small pure mapping table gets reimplemented per crate" convention
//! `cratestack-proto::casing` already documents for itself.
//!
...
//! fine for `.proto` text but not for a wire encoder, which needs to know
//! *how* to encode a reference — varint for an enum, length-delimited +
//! recursive encode for a message).
...
/// as its lowercase tag so `grpc-runtime.dart.j2`'s data-driven
/// `encodeMessage`/`decodeMessage` can switch on it directly.
...
pub(crate) enum GrpcWireKind {
    ...
    /// Dart runtime as a built-in message shape rather than a generated
    /// descriptor table.
    Timestamp,
    ...
    /// `PageOf<M>`, ...) — length-delimited, recursively encoded/decoded
    /// against that message's own descriptor table.
    Message,
}
...
pub(crate) struct MappedWireField {
    ...
    /// codegen time and to name the generated Dart symbol.
...
/// A single field on a [`super::messages::GrpcMessageView`] — the Dart
/// runtime's unit of data-driven encode/decode. `repeated` covers
/// `TypeArity::List`; every other arity (`Required`/`Optional`) is
/// presence-tracked the same way on the wire (proto3 explicit `optional`,
/// `docs/design/protobuf.md` §4.4 — CrateStack's universal-optional rule
/// makes the Required/Optional distinction a documentation-only concern,
/// not a wire behavior one), so this descriptor doesn't carry arity beyond
/// the repeated/scalar split.
...
pub(crate) struct GrpcFieldDescriptor {
    ...
    /// `PageInfo.hasNextPage`/`hasPreviousPage`, matching the hand-written
    /// `PageInfo` class in `models.dart.j2`, which declares them
    /// `required` rather than nullable). For those, an absent wire tag
    /// means the real value *is* the type's zero value (`false`/`0`/`""`),
    /// not "not set" — decoding must fill it in rather than leaving the
    /// property absent from the decoded map. Every other field uses
    /// explicit presence (`docs/design/protobuf.md` §4.4): absent
    /// genuinely means "not provided", so the property is left out of the
    /// decoded map, matching a `null` value flowing into the optional
    /// Dart field it's decoded into.
