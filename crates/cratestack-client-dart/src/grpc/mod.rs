//! gRPC template-context building — ticket #210, the Dart counterpart to
//! `cratestack-client-typescript::grpc` (ticket #172's gRPC-Web client).
//! Model CRUD only, matching what `cratestack-grpc`'s macro-generated
//! tonic service actually exposes (ticket #171): procedures and
//! server-streaming are not wired into the generated gRPC service at all,
//! so there is nothing for this generator to bind a Dart method to for
//! either — see `crates/cratestack-grpc/src/lib.rs`'s module doc for the
//! authoritative scope statement this generator mirrors.
//!
//! Unlike the TypeScript gRPC-Web client, the Dart client speaks real
//! HTTP/2 gRPC via `package:grpc`'s `ClientChannel`/`ClientMethod` — no
//! manual frame/trailer parsing needed here, `package:grpc` handles that.
//! What both generators share is the hand-rolled protobuf *message* codec
//! (varint/fixed64/length-delimited, `google.protobuf.Timestamp`, enum
//! tables) — that's what this module and its Dart runtime template port.
//!
//! Sub-concerns, one per file to stay under the repo's 200-LoC
//! convention:
//! - [`wire`] — field-level wire-kind mapping (`.cstack` scalar -> how the
//!   Dart runtime encodes/decodes it).
...
use crate::naming::{enum_name_set, primary_key_field};
...
/// carries). Every field the Dart runtime's `encodeMessage`/`decodeMessage`
...
/// Builds the gRPC-specific slice of the template context. Returns `None`
/// for non-`Grpc` transports (callers skip attaching it); errors when a
/// `Grpc`-transport schema has no `.pb.lock` (`MissingPbLock`,
...
) -> Result<Option<GrpcContext>, DartGeneratorError> {
    if schema.transport != cratestack_core::TransportStyle::Grpc {
        return Ok(None);
    }
    let pb_lock = pb_lock.ok_or(DartGeneratorError::MissingPbLock)?;
    let package = pb_lock
        .package
        .clone()
        .ok_or(DartGeneratorError::MissingPbLockPackage)?;
...
/// list-projection controls. Deliberately excludes `where_expr`/`or` (raw
/// predicate query strings) and `filters` (`repeated RpcListPredicate`, an
/// advanced structured-predicate builder) and `include_fields` (a
/// `map<string, StringList>`, which needs protobuf map-entry encoding
/// this pass doesn't implement) — same reduction ticket #172 shipped for
/// TypeScript. Every field left out here simply never gets set on the
/// wire, which decodes on the server as "not provided" (proto3 explicit-
/// presence `None`), the same as a REST/RPC caller who never passed those
/// query parameters.
