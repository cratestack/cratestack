//! Per-model gRPC-Web method descriptors: the wire path and method name
//! each CRUD verb dials, plus which message names its request/response
//! bodies encode/decode against. `docs/design/protobuf.md` §4.6's
//! `/<package>.Api/<MethodName>` shape, `<package>` from the schema's
//! locked `.pb.lock` and `<MethodName>` from
//! `cratestack_proto::op_id_to_method_name` — the same derivation the
//! Rust server's generated tonic service and `cratestack-proto`'s
//! `.proto` `service` block use, so a `grpcurl`-discovered method name and
//! this generated client's method name are never out of sync.
...
use crate::types::{model_allows_create, primary_key_field, ts_type};
...
/// macro-generated service (`build_list_arm`) always wraps the final
/// gRPC response in `pb::PageOf<Model>` even for unpaged models
/// (synthesizing default `PageInfo` there). So unlike REST/RPC's
/// `ModelApiView::list_return_type`, there is no unpaged/paged branch
/// here — no `paged` field on this view at all.
...
pub(crate) fn build_grpc_model_view(package: &str, model: &Model) -> GrpcModelView {
    ...
    GrpcModelView {
        ...
        accessor: crate::naming::pluralize(&to_camel_case(&model.name)),
        api_name: format!("{}Api", model.name),
        primary_key_type: ts_type(&primary_key.ty, &std::collections::BTreeSet::new()),
