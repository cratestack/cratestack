//! cratestack#345 regression guard: the generated TypeScript client's REST
//! route for a model must match the server's real Axum route registration
//! exactly, including for model names the old `split_words`-based
//! `to_snake_case` in `crate::naming` diverged on (a literal `_` boundary).
//!
...
fn generated_client_source() -> String {
    let schema =
    ...
    package
        .files
        .iter()
        .find(|file| file.file_name == "src/client.ts")
        .expect("client.ts should be generated")
        .contents
        .clone()
}
...
fn underscore_containing_model_route_matches_the_canonical_server_algorithm() {
    let client = generated_client_source();
    ...
    assert!(
        client.contains(&user_group_route),
        "expected client.ts to reference {user_group_route}, got:\n{client}"
    );
    assert!(
        client.contains(&user_underscore_group_route),
        "expected client.ts to reference {user_underscore_group_route}, got:\n{client}"
    );
    ...
    // tests.
    assert!(client.contains("\"/user_groups\""));
    assert!(client.contains("\"/user__groups\""));
}
