...
//! had no equivalent guard, so `cratestack generate-typescript` against
//! such a schema aborted with
//!
//!     thread 'main' panicked at src/views.rs:120:
...
fn a_composite_primary_key_is_an_error_not_a_panic() {
    ...
    let error = generate_package(&schema, &TypeScriptGeneratorConfig::default())
        .expect_err("a composite-PK schema must be rejected, not generated");

    let TypeScriptGeneratorError::CompositePrimaryKeyUnsupported(message) = &error else {
...
fn a_single_scalar_primary_key_still_generates() {
    ...
    assert!(
        package.files.iter().any(|f| f.file_name == "src/client.ts"),
