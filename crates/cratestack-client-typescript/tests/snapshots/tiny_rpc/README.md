...
const client = new TinyRpcClientClient("https://api.example.com");
```
...
export function createClient(accessToken: string | null) {
  return new TinyRpcClientClient(process.env.EXPO_PUBLIC_API_ORIGIN!, {
    basePath: "/api",
    headers: {
...
```ts
import { Decimal } from "tiny-rpc-client";
const item = await client.widgets.get(id);
