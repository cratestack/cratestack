// Single-CBOR-item structural walker — the low-level half of the
// `application/cbor-seq` boundary scanner (issue #277, see
// `./cbor-seq` for the stateful, chunk-buffering half that consumes
// this). Ported from `crates/cratestack-client-rust/src/streaming.rs`'s
// use of `minicbor::Decoder::skip` deliberately, not reimplemented from
// a CBOR spec reading — that file's own doc comment explains the risk:
// a subtly wrong walk (mishandling indefinite-length arrays/maps/byte
// strings, or the tag-argument decoding) would silently misparse item
// boundaries rather than fail loudly.
//
// Internal to the generated package (not re-exported from `./index`) —
// `./cbor-seq` is the public surface for boundary-scanning.
...
function skipCount(bytes: Uint8Array, start: number, count: number, itemsPerElement: number): number {
  let cursor = start;
