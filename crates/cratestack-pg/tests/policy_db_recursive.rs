...
//      rejects on explicit-id inserts. Fixed by switching to
//      `GENERATED BY DEFAULT`.
//   2. After (1) is unblocked, the "cross-org update should fail"
//      assertion used to fail — not because of update-policy drift in
//      the evaluator, but because the seed itself was wrong: it gave
//      user 2 ("other") an actual membership on project 1 (the same
//      project task 1 belongs to). `Task`'s
//      `@@allow("update", project.memberships.some.user.email ==
//      auth().email)` clause has no org-slug condition (unlike its read
//      policy, which requires `project.organization.slug ==
//      auth().orgSlug` in addition to membership), so `some` correctly
//      grants update access to any project member regardless of org —
//      "other" being seeded as a real member of project 1 legitimately
//      satisfied that clause. Confirmed live: before the fix,
//      `cool.task().update(1).run(&other)` returned
//      `Ok(Task { id: 1, title: "Blocked update", .. })` instead of
//      `Forbidden`. Fixed (2026-08 audit) by removing that membership
//      row from the seed so "other" has no relationship to project 1 at
//      all, matching the test's actual intent.
#[tokio::test]
async fn db_backed_recursive_relation_policies_cover_quantifiers_and_create_checks() {
    ...
    // NOTE: user 2 ("other") is deliberately NOT a member of project 1
    // here. `Task`'s `@@allow("update", project.memberships.some.user
    // .email == auth().email)` clause has no org-slug condition (unlike
    // the read policy), so `some` grants update access to ANY project
    // member regardless of org — giving "other" a membership on
    // project 1 would make the "cross-org update should fail" check
    // below fail correctly per that policy, not because of a framework
    // bug (see the block comment above this test for how this was
    // diagnosed).
    cratestack::sqlx::query(
        "INSERT INTO memberships (id, project_id, user_id, active, blocked) VALUES (1, 1, 1, TRUE, FALSE), (3, 2, 1, TRUE, FALSE), (4, 2, 3, TRUE, FALSE), (5, 3, 2, TRUE, FALSE), (6, 4, 1, TRUE, TRUE), (7, 5, 1, FALSE, FALSE)",
    )
    .execute(pool)
    .await
