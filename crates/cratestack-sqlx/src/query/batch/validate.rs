...
//! return `CratestackError::Validation` so the wrapping `run` fn returns
//! before opening a transaction.

use std::hash::Hash;

use cratestack_core::{BATCH_MAX_ITEMS, CratestackError, find_duplicate_position};

use crate::SqlValue;

pub(super) fn validate_batch_size(len: usize) -> Result<(), CratestackError> {
...
pub(super) fn reject_duplicate_pks<K: Eq + Hash + Clone>(
    keys: &[K],
...
pub(super) fn reject_duplicate_sql_values(values: &[SqlValue]) -> Result<(), CratestackError> {
