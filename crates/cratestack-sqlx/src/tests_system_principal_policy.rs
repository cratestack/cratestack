...
//! Runtime semantics of the `auth().isSystem()` policy predicate
//! (issue #486 / ADR 0038 blocker B1).
//!
//! Exercises the real evaluators — `render_read_policy_sql` (the read
//! path `find_unique`/list scoping use, and the same entry point
//! `tests_read_policy_predicates.rs` uses) and
//! `evaluate_input_predicate` (the create path's in-process evaluator)
//! — so these are assertions about what a given caller's policy
//! actually resolves to, not about a re-implementation. No DB
//! connection needed; PG-backed coverage that a system read actually
//! comes back from a real table lives in
//! `cratestack-pg/tests/system_principal.rs`.
//!
...
//! the feature has degenerated into the blanket bypass flag design
...
/// Granting half: a system caller satisfies the `isSystem()` arm
...
/// Denying half: the *same policy* for a request-derived caller
/// collapses the `isSystem()` arm to `FALSE` and falls through to the
/// ownership check, which still binds the caller's own subject id. The
/// end user is not elevated by the mere presence of the arm — this is
/// the "a non-system caller is unaffected" acceptance criterion.
#[test]
...
/// THE FAIL-CLOSED CASE — the most important test in this file.
///
/// A model whose policies never mention `isSystem()` must not become
/// writable/readable just because the caller holds a `SystemContext`.
/// This is the property that distinguishes "a principal policies name"
/// from "a bypass flag": the system context carries no `subjectId`
/// claim, so the ownership predicate renders `FALSE` for it, exactly as
/// it would for any other caller missing that claim.
#[test]
...
/// action is not writable/readable by system code either. Default-deny
/// is unconditional and the system principal does not weaken it.
#[test]
...
mod create_path {
    ...
    /// Fail-closed on the create path: a system caller does not satisfy
    /// an ownership predicate it was never named in.
    #[test]
