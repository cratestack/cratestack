...
pub(super) fn parse_policy_term(
    ...
    // SPIKE (`spike/b1-internal-actions`): `auth().isSystem()`. Must be
    // recognised *before* `parse_builtin_policy_call`, which finds the
    // first `(` in the term — for `auth().isSystem()` that is `auth`'s,
    // so it would report "policy function `auth` requires a string
    // literal argument". Whitespace is normalized because the AST
    // splitter only trims term edges.
...
/// SPIKE: recognises `auth().isSystem()` (and the bare `isSystem()`
/// shorthand) with any interior whitespace.
