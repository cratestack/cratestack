//! SPIKE (`spike/b1-internal-actions`): compile-time lowering of
//! `auth().isSystem()`.
//!
...
//! in front of `parse_builtin_policy_call`, which would otherwise
//! claim it and error), and — the fail-closed half — that a policy
//! which never writes `isSystem()` never emits that predicate.
