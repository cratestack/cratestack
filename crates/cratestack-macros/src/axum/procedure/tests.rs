...
///
/// **Re-pinned a second time, for cratestack#512.** The `invoke_with_db`
/// closure changed from `|| async move { registry.ticks(&db, &call_ctx,
/// call_args).await }` to `|authorized| async move { registry.ticks(&db,
/// &call_ctx, call_args, authorized).await }` — `invoke_with_db` now hands
/// the closure an `Authorized` witness (constructible only by its own
/// internal `authorize_with_db` call) and threads it into the
/// `ProcedureRegistry` method call, which is what makes calling that
/// method directly, without going through `invoke_with_db`/
/// `authorize_with_db` first, fail to compile. Same "re-pin, don't
/// paper over" discipline as the #415 note above: this dispatch tokens
/// change is real and intentional, unrelated to cratestack#283's
/// stream-vs-buffered distinction.
///
/// **Re-pinned a third time, for the `AuthProvider` extensions plumbing.**
/// `request_context(...)`'s call site now passes a trailing
/// `&client_ip_ctx.extensions` argument so `RequestContext::extensions`
