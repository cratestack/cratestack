...
//! dispatch-tail helpers it composes — it has not moved since, so this
//! test still passing is the "diff of zero lines" proof this ticket's
//! acceptance criteria ask for (mirrors `crate::procedure::tests`, the
//! analogous guard for cratestack#282's trait-method change).
