...
// Json wrapper
//
// Schema-declared `Json` fields need a model-struct field type that's the same
// on every backend so the same struct compiles on server and on embedded
// (including `wasm32-unknown-unknown`, which can't depend on sqlx). This
// newtype lives in `cratestack-core` so every backend can reference it
...
/// Wire-shape the schema generates for. Picked once per schema (via the
/// top-level `transport rest|rpc` directive) so generated servers and
/// clients only carry one binding's worth of surface.
...
/// `docs/design/rpc-transport.md` for the full design — in short, an op is
/// the dispatch unit shared by every RPC binding (HTTP unary, HTTP batch,
/// HTTP stream, WebSocket). The macro emits one `OpDescriptor` per CRUD
/// verb and per procedure when `Schema.transport == TransportStyle::Rpc`.
///
/// REST schemas continue to emit `RouteTransportDescriptor` instead;
/// nothing emits both.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct OpDescriptor {
    /// Stable dotted id, e.g. `"model.User.list"` or `"procedure.publishPost"`.
    /// This is the only dispatch key — same string appears in URLs
    /// (`POST /rpc/:op_id`), in batch/WS `Request.op` fields, and in
    /// generated client SDK call sites.
    ...
    /// Schema-level name of the output type. Empty string when the op
    /// returns nothing (e.g. `delete` with no echo).
    pub output_ty: &'static str,
    /// Whether the op can be safely retried without an idempotency key.
    /// True for reads and pure procedures; false for mutations.
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OpKind {
    /// One input, one output. The common case — every CRUD verb and every
    /// non-streaming procedure.
    Unary,
    /// One input, a finite sequence of outputs. Used for `@stream`
    /// procedures and (future) streamed `list`. Terminates server-side.
    Sequence,
    /// One input, an open-ended sequence of outputs ended only by client
    /// cancellation or disconnect. WebSocket-only — see §3.4 of the
    /// design doc. Fire-and-forget: no cursors, no replay buffer.
    Subscription,
}
...
//
// These are the protocol shapes both the server binding (`cratestack-axum::rpc`)
// and every generated client (`cratestack-client-rust`, the TS/Dart
// generators) agree on. They live here in `cratestack-core` so clients can
// depend on a single source of truth without pulling in axum.
//
// Server-only helpers (codec-aware encoding, axum response post-processing,
// batch frame assembly) stay in `cratestack-axum::rpc`. This module owns
// only the wire shapes and the CoolError → gRPC-style code mapping.
// -----------------------------------------------------------------------------

pub mod rpc {
    //! Wire shapes for the `transport rpc` binding. See
    //! `docs/design/rpc-transport.md` for the full design.

    use serde::{Deserialize, Serialize};

    use crate::{CoolError, CoolErrorResponse};
    ...
    impl RpcErrorBody {
        ...
        /// Translate a REST-style [`CoolErrorResponse`] into the RPC error
        /// body. The `code` field is mapped from screaming-snake to
