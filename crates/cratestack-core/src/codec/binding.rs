...
/// wire. A body sealed for one `(method, route, path_params, query,
/// schema)` fails to open under any other, which defeats cross-endpoint
/// replay; a response is bound to its request's digest and its status,
